...
System.out.println("\n" + Color.colorize("You encounter a " + enemy.getDisplayName() + " in " + loc.name + "!", color));
    ...
    private void encounter(Location loc) {
        ...
        while (player.hp > 0 && enemy.isAlive()) {
            ...
            // player.applyPassive(); // Removed because Hero does not have applyPassive()
    ...
    private boolean checkDodge(Hero player) {
        if (player instanceof Archer && random.nextInt(100) < 20) {
            ...
            System.out.println(Color.colorize("Assassin dodges the enemy’s next attack!", Color.GREEN));
            return true;
        }
        return false;
    }
    ...
    private void applyEnvironmentalEffects(Hero player, Enemy enemy, Location loc) {
        ...
        switch (loc.environmentalEffect) {
            ...
            case "Thick Fog":
                ...
                // This will require modifying attack logic to incorporate an accuracy check.
                // For now, I will just print the message.
                break;
            case "Glowing Spores":
