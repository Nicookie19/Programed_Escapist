...
public abstract class Hero implements Serializable {
    ...
    public void updateStatusEffects() {
        for (Iterator<StatusEffect> it = statusEffects.iterator(); it.hasNext();) {
            ...
            if (!effect.isActive()) {
                it.remove();
                resetStat(effect.getTargetStat());
            }
        }
    }
    ...
    private void resetStat(String targetStat) {
        if (targetStat.equals("damage")) {
            // Recalculate damage based on your logic (e.g., equipment, level)
            // Example: minDmg = baseMinDmg; maxDmg = baseMaxDmg;
            // Adjust based on your Hero damage calculation
            // Placeholder: Assume damage is reset via equipment or level
