    ...
    public void useSkill(int skillIdx, Enemy enemy) {
        ...
        switch (skillIdx) {
            ...
            case 3: // Regression
                ...
                break;
            case 4: // Penetration
                if (penetrationCooldown == 0 && mana >= 20) {
                    int baseDamage = minDmg + random.nextInt(maxDmg - minDmg + 1);
                    int damage = (int)(baseDamage * 2.2 * multiplier);
                    if (random.nextInt(100) < critChance) {
                        damage *= 2;
                        System.out.println("Critical Penetration ignores armor and deals " + damage + " damage!");
                    } else {
                        System.out.println("You penetrate defenses, ignoring armor, and deal " + damage + " damage!");
                    }
                    enemy.receiveDamage(damage);
                    mana -= 20;
                    penetrationCooldown = 4;
                } else {
                    System.out.println("Penetration is on cooldown or insufficient mana! Using normal attack.");
                    super.useSkill(1, enemy);
                }
                break;
            default:
